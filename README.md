...
In production the backend serves the built frontend directly, so the whole app is one service on one URL (no separate frontend host, no CORS setup).

1. Push this repo to GitHub.
2. On [Render](https://render.com), click **New > Blueprint** and point it at the repo — it picks up `render.yaml` and provisions a free Web Service using the included `Dockerfile`.
3. When prompted, set the `NEWS_API_KEY` environment variable (your key from newsapi.org). `JWT_SECRET` is auto-generated by the blueprint.
4. Deploy. Render builds the frontend and backend into one Docker image and serves everything from the URL it gives you.

Note: the free tier has no persistent disk, so `users.db` (news cache + any registered accounts) resets whenever the service redeploys or spins down from inactivity — fine for a personal project, but accounts won't survive long-term without a paid disk.
